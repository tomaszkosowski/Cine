...
public static class DependencyInjectionExtensions
{
    ...
    private static IServiceCollection AddSagas(this IServiceCollection services)
    {
        ...
        {
            cfg.AddSagaStateMachine<PaymentSaga, PaymentState>().InMemoryRepository();

            cfg.AddConsumer<CreatePaymentConsumer>();
            cfg.AddConsumer<ProcessPaymentConsumer>();
            cfg.AddConsumer<ConfirmPaymentConsumer>();
            cfg.AddConsumer<CancelPaymentConsumer>();

            cfg.UsingInMemory((context, config) =>
            {
                config.UseConsumeFilter(typeof(UnitOfWorkConsumerFilter<>), context);
    ...
    {
        services.AddSingleton<ReservationConfirmedIntegrationEventHandler>();
    ...
    {
        services.AddRefitClient<ITicketsApiClient>()
            .ConfigureHttpClient(client => client.BaseAddress = new Uri(options.TicketsApiUrl));
    ...
    private static IApplicationBuilder UseIntegrationEvents(this IApplicationBuilder appBuilder)
    {
        ...
        {
            const string queueName = "sales";

            eventBus.SubscribeAsync(queueName,
                services.GetRequiredService<ReservationConfirmedIntegrationEventHandler>()).Forget(logger);
        });
